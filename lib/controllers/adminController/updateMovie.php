...
//    }
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if(!isset($_POST['cartel']) || empty($_POST['cartel']) ){
            $cartelCheck = true;
        }
        
        //If any error ocurred then redirects
        if($titleCheck==true || $generoCheck==true || $paisCheck==true || $anyocheck==true || $cartelCheck==true || $idCheck==true){
            header('Location: ../../../pages/adminPages/updateMoviePage.php?titulo='. trim($_POST['titulo']).'&genero='.$_POST['genero'].'&pais='.$_POST['pais'].'&anyo='.$_POST['anyo'].'&cartel='.$_POST['cartel']);
        }
        
        $idCheck = filter_input(INPUT_POST, 'id', FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        updateMovie($idCheck, $titulo, $genero, $pais, $anyo, $cartel);
